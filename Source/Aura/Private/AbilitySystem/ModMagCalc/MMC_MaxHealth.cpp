...
UMMC_MaxHealth::UMMC_MaxHealth()
{
	...
	VigorDef.bSnapshot = false; //If true, we capture it as soon as GE is created

	//Our MMC needs an array of attributes to capture
	RelevantAttributesToCapture.Add(VigorDef); //When modifier is executed when GE is applied, Vigor will be captured from the target at calculation time
}
...
float UMMC_MaxHealth::CalculateBaseMagnitude_Implementation(const FGameplayEffectSpec& Spec) const
{
	...
	float Vigor = 0; //This value will be filled by GetCapturedAttributeMagnitude
	GetCapturedAttributeMagnitude(VigorDef, Spec, EvaluationParameters, Vigor);
	//Usually these values get clamped for not getting negative values
	Vigor = FMath::Max<float>(Vigor, 0.f); //If Vigor is less than 0, we'll set it to 0
